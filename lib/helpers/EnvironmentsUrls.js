import { configWeb } from "..";
const EnvironmentsUrls = {
    local: {
        SRV_DIRECTORY: `http://${window.location.pathname}:7001`,
        SRV_CALENDAR: `http://${window.location.pathname}:7011`,
        SRV_PROXY_OLD: `http://${window.location.pathname}:4430`,
        SRV_STORAGE: `http://${window.location.pathname}:4000`,
        SRV_CHAT: `http://${window.location.pathname}:7012`,
        SRV_CONNECTOR: 'https://connector-dev.adopus.no',
        SRV_ARTIFACT: '',
        SRV_ADVOCA: `http://${window.location.pathname}:4433`,
        SRV_PROXY: `http://${window.location.pathname}:7003`,
    },
    dev: {
...
export function environmentUrls() {
    var _a;
    const env = configWeb('ENVIRONMENT_TO_OPERATE', 'no_env');
    return env !== "no_env" && ((_a = window.__ENV) === null || _a === void 0 ? void 0 : _a['DEVELOPMENT']) && EnvironmentsUrls[env] || undefined;
}
